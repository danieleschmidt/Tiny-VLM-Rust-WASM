//! Generation 2: Fixed Robust & Reliable VLM Demo
//! 
...
impl fmt::Display for VLMError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            VLMError::SecurityError(msg) => write!(f, "Security error: {}", msg),
            VLMError::ResourceError(msg) => write!(f, "Resource error: {}", msg),
            VLMError::NetworkError(msg) => write!(f, "Network error: {}", msg),
            VLMError::ValidationError(msg) => write!(f, "Validation error: {}", msg),
        }
    }
}
...
impl VLMError {
    ...
    pub fn processing_error(msg: &str) -> Self {
        Self::ProcessingError(msg.to_string())
    }
    
    pub fn security_error(msg: &str) -> Self {
        Self::SecurityError(msg.to_string())
    }

    pub fn is_retryable(&self) -> bool {
        matches!(self, VLMError::ProcessingError(_) | VLMError::NetworkError(_))
    }
}
...
impl ValidationFramework {
    ...
    pub fn validate_image(&self, image_data: &[u8]) -> Result<()> {
        if image_data.is_empty() {
            return Err(VLMError::invalid_input("Image data cannot be empty"));
        }
        
        if image_data.len() > self.image_size_limit {
            return Err(VLMError::invalid_input("Image size exceeds limit"));
        }
    ...
    pub fn validate_text(&self, text: &str) -> Result<()> {
        if text.is_empty() {
            return Err(VLMError::invalid_input("Text input cannot be empty"));
        }
        
        if text.len() > self.text_length_limit {
            return Err(VLMError::invalid_input("Text too long"));
        }
        
        Ok(())
    }
    
    pub fn validate_security(&self, text: &str) -> Result<()> {
        ...
        for pattern in malicious_patterns {
            if text.contains(pattern) {
                return Err(VLMError::security_error("Potentially malicious content detected"));
            }
        }
...
impl CircuitBreaker {
        ...
        F: FnOnce() -> Result<T>,
    {
        ...
        if !self.should_allow_request() {
            return Err(VLMError::processing_error("Circuit breaker is open"));
        }
...
impl Generation2VLM {
    pub fn new(config: Generation2Config) -> Result<Self> {
        ...
        if config.vision_dim == 0 || config.text_dim == 0 || config.hidden_dim == 0 {
            let error = VLMError::ConfigError("Invalid dimensions in config".to_string());
    ...
    pub fn infer_with_robustness(&mut self, image_data: &[u8], text_prompt: &str) -> Result<String> {
        ...
        let text_prompt_owned = text_prompt.to_string();
        
        // Run inference with circuit breaker protection
        let result = {
            let vlm_ref = self as *mut Self;
            self.circuit_breaker.call(|| unsafe {
                (*vlm_ref).infer_with_retry(&image_data_owned, &text_prompt_owned)
            })
        };
    ...
    fn infer_with_retry(&mut self, image_data: &[u8], text_prompt: &str) -> Result<String> {
        for attempt in 0..self.config.max_retry_attempts {
            ...
            match self.infer_internal(image_data, text_prompt) {
                ...
                Err(error) => {
                    if !error.is_retryable() || attempt == self.config.max_retry_attempts - 1 {
                        return Err(error);
                    }
        ...
        Err(VLMError::processing_error("All retry attempts failed"))
    }
    ...
    fn infer_internal(&mut self, image_data: &[u8], text_prompt: &str) -> Result<String> {
        ...
        if self.config.enable_security_checks {
            self.perform_security_checks(text_prompt)?;
        }
    ...
    fn validate_inputs(&self, image_data: &[u8], text_prompt: &str) -> Result<()> {
        ...
        // Validate text input
        self.validator.validate_text(text_prompt)?;
        
        self.logger.log(LogLevel::Debug, "VLM", "Input validation passed");
        Ok(())
    }
    
    fn perform_security_checks(&self, text_prompt: &str) -> Result<()> {
        self.logger.log(LogLevel::Debug, "VLM", "Performing security checks");
        
        // Security validation
        self.validator.validate_security(text_prompt)?;
    ...
    fn generate_robust_response(&self, image_data: &[u8], text_prompt: &str) -> Result<String> {
        ...
        if response.is_empty() {
            return Err(VLMError::processing_error("Generated empty response"));
        }
...
fn main() -> Result<()> {
    ...
    let config = Generation2Config::default();
    ...
    let recent_logs = vlm.get_recent_logs(5);
    for entry in recent_logs.iter() {
